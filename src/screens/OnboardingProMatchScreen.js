...
const OnboardingProMatchScreen = () => {
  ...
  const scrollToTheNextProMatchSlide = async () => {
    if (currentProMatchSlideIndex < proOnboardingMatchData.length - 1) {
      proSlidesMatchRef.current.scrollToIndex({
        index: currentProMatchSlideIndex + 1,
      });
    } else {
    ...
    <View
      ...
      <View style={{display: 'flex'}}>
        <FlatList
          ...
          onScroll={Animated.event(
            [{nativeEvent: {contentOffset: {x: proMatchScrollX}}}],
            {
      ...
      <TouchableOpacity
        ...
        <Text
          ...
          {currentProMatchSlideIndex === proOnboardingMatchData.length - 1
            ? 'Get Started'
            : 'Next'}
        </Text>
      </TouchableOpacity>
